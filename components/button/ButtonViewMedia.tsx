...
export function ButtonViewMedia({
  ...
  return (
    <button className="flex flex-row items-center justify-center px-4 py-2 bg-[#FFFFFF]/10 rounded-md gap-1.5 transition-all [&_svg:not([class*='size-'])]:size-4.5 [&_svg]:shrink-0 ">
      {children}
...
export function ButtonViewMediaSteam({
  ...
  return (
    <button className="flex flex-row items-center justify-center px-4 py-2 bg-stream rounded-md gap-1.5 transition-all [&_svg:not([class*='size-'])]:size-4.5 [&_svg]:shrink-0 ">
      {children}
      <p className="text-white font-regular text-sm">{text}</p>
    </button>
  );
